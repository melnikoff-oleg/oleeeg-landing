...
## /viral-reels, the reel search and the reel library (2026-08-20)
...
- **Both pages are deliberately bare (2026-08-20).** No header, no wordmark, no hero, no example chips, no copy, no ResourceFooter: `/viral-reels` is a search box, the recency filter and ten results, and `/viral-reels/browse` is the filters and the list. That is why everything under `/viral-reels` is excluded from `SHELL_ROUTES` and `FOOTER_ROUTES` in `tests/e2e/routes.ts`; both stay in `ROUTES` so they must still answer 200, render exactly one `h1` and log no console errors. The single link each page carries to the other is the one exception to "nothing but the results", and it is there because the library page is otherwise unreachable. The `h1` is `sr-only`, because a page still owes a screen reader one heading and this one has nothing on it to use.
