...
- Free-call window (`free-call.spec.ts`, 59-63): the booking link renders exactly once on all 30 routes (the shared `ROUTES` list plus the six filmed pages, which that list deliberately excludes), is `target=_blank` + `rel=noopener`, and the rendered copy carries free / the window / not-a-sales-call; plus the no-dash rule and mobile 44px tap target + 16px body copy. **Delete this spec when the window closes.**
...
- `src/components/free-call-cta.tsx` — `FreeCallCta`, the free 30 minute call offered on **every one of the 30 public routes** (see the dated section below). Copy is in `src/lib/free-call.ts`, one source of truth, because the card has two implementations: this Tailwind one, and an inline-styled twin (`FreeCallPanel`) inside `filmed-page-outro.tsx` for the six filmed pages, which can carry neither Tailwind nor client JS. Placement rule: **after the hero, never inside it** so the fold still hands over the page's promised asset (`RepoCta`) and the hero CTA stays the first external link in `<main>`, which `hero-cta.spec.ts` asserts. The title is solid `text-white`, NOT `.text-metallic`: the metallic gradient's dark stops land on the end of a short line and dimmed the one word that has to land ("free").
...
## Free-call window (2026-08-13, TIME-BOXED, remove after 2026-08-23)

A short customer-development window: a free 30 minute call with Oleg, booked at
`calendly.com/boldane/free-30min-consult`, offered on **every one of the 30 public
routes**. The point is not lead-gen, it is that Oleg wants to hear what his
audience is actually stuck on so he can make better videos, and the copy says
exactly that ("not a sales call. i just want to learn."). Third-grade language,
badge + title + limit + button readable in about ten seconds.

**This is the one deliberate exception to the Boldane presence policy.** It is not
a Boldane pitch (it is Oleg offering help, and it pitches nothing), so it sits
alongside a page's existing `BoldaneCta` or `boldaneCredit` without breaking the
one-pitch-per-page rule. It is also why `/marketing-brain`, which by design has no
Boldane presence at all, still carries it (in the empty state only, below the
minimal hero, so the Jobs-style hero keeps its short body copy and the card
disappears once a conversation starts).

Wiring, 30 routes: `ResourcePageShell` (the 16 `claude-*` pages), the homepage
(between results and video), the six bespoke pages that hand-inline their layout
(`ads-ai`, `high-converting-website`, `60k-linkedin-post`, `5-levels-ai`,
`opus-5`, `marketing-brain-knowledge`), `/marketing-brain`, and `FilmedPageOutro`
for the six filmed pages. The filmed pages still ship **zero client JS** (172 B
First Load), which the inline-styled twin exists to preserve.

**To close the window:** `grep -rn "FreeCallCta\|FREE_CALL\|free-call" src tests`,
delete the mounts, `src/components/free-call-cta.tsx`, `src/lib/free-call.ts`, the
`FreeCallPanel` in `filmed-page-outro.tsx` and `tests/e2e/free-call.spec.ts`, then
`npm run test:update`. A stale "this week and next week only" is worse than no
line at all. To extend instead, edit `limit` in `src/lib/free-call.ts`.
