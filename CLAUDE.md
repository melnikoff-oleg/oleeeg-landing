...
## /viral-reels, the three reel pages (2026-08-20)
...
- **All three pages are deliberately bare (2026-08-20).** No header, no wordmark, no hero, no copy, no ResourceFooter. That is why every `/viral-reels*` route is excluded from `SHELL_ROUTES` and `FOOTER_ROUTES` in `tests/e2e/routes.ts`; all three stay in `ROUTES` so they must still answer 200, render exactly one `h1` and log no console errors. The `h1` is `sr-only`, because a page still owes a screen reader one heading and none of these has anything on it to use. The one piece of chrome they share is `ReelNav`, three links, which replaced the single ad-hoc link between search and browse: that worked for two pages and stops working at three.
- **The slugs are flat, and `/viral-reels/browse` redirects (2026-08-20).** Oleg asked for one path segment per page, so the library moved from `/viral-reels/browse` to `/viral-reels-browse` and `next.config.ts` carries a permanent redirect, because the old URL was linked and is in a sitemap Google already fetched. The API routes keep their folder (`/api/viral-reels/browse`); only the page slugs are flat.
...
### /viral-reels-ideas, the chat (2026-08-20)

- **It is an agent loop, not a retrieval-then-answer.** Unlike `/api/marketing-brain/chat`, which retrieves once and writes, `/api/viral-reels/ideas` lets `claude-sonnet-5` decide what to look up and runs the lookup itself, up to `MAX_ROUNDS = 6` with a 210-second wall clock inside a `maxDuration` of 300. Web search and web fetch are Anthropic's own server tools and resolve inside the API request; only the three reel tools come back to this route.
- **The model gets three tools and no more, in `src/lib/reels/tools.ts`.** `library_overview`, `search_reels`, `top_reels`. It never picks a table, a column or a filter shape: it fills a schema and this module turns that into one of three hand-written reads. A prompt injected through a reel caption or a fetched page can change what the model *says*; it cannot change what it can *reach*. Nothing in the feature writes to Supabase.
- **`library_overview` must be called first, and the prompt insists on it.** Without it the model answers a photography brand by describing the photography reels it wishes existed. With it, it can see the library is 65 accounts weighted to comedy, fitness, food and effects, say so in one line, and pivot to the cross-niche play, which is the better answer anyway. It is one four-column read of the whole table, cached in module memory for 30 minutes behind a shared in-flight promise.
- **Two reels per account, always.** `search_reels` for "using AI tools to get work done faster" returned six reels, four of them the same creator: one person's habits presented as evidence about a niche. Every tool now over-fetches (six times the wanted count for search, up to eight times for an effort-filtered `top_reels`) and thins the ranked list to `DEFAULT_PER_ACCOUNT = 2`, keeping the best-ranked of each.
- **A tool that returns fewer reels than asked says why.** Silence is the dangerous case: three reels back from a request for six reads as "the library only has three", and the model writes that down as fact. `shortfallNote` names whichever filter did the cutting so the model can widen and retry.
- **`src/lib/reels/effort.ts` is a heuristic and is meant to be.** There is no production-cost column and adding one means a paid re-analysis of all 1033 reels. Instead it reads `shots` (stored as text, holds an integer), `duration_sec` and the analysis prose, and returns easy / medium / hard. Three rules earn their keep: AI markers are checked *before* the expensive-capability list, because a shot that used to need a crew is now a prompt and lands at medium however impossible it looks; shot count alone can never mean hard, since a long cut list is editing time, not money (a 26-shot reel of two men crushing fruit was being called hard); and gear or a second person floors at medium rather than hard (two men talking about flags was hard because the write-up mentioned studio lighting). Distribution over the live library is 29 percent easy, 52 medium, 19 hard.
- **The model cites reels as `[[reel:SHORTCODE]]` and the client swaps them for cards.** The route streams a `reels` frame with the full rows as each tool returns, so a citation always has its card data before the text mentioning it arrives. `ReelCite` is built entirely from spans, never divs: it renders inside whatever markdown element the citation landed in, usually a `<p>`, and a div in a p is invalid HTML that React reparents at hydration. An uncited shortcode falls back to a plain Instagram link rather than leaving raw brackets in a sentence.
- **Per-IP cap is 20 a day**, in its own `reel-ideas` bucket. Each question is several Sonnet turns plus web search, the most expensive thing on the site.
- **The conversation is not persisted.** Reloading the page starts over, by design, for the MVP.
- **Every way of stopping that is not a finished answer is said out loud.** The worst outcome this page has is a half answer that renders as a whole one, because the visitor then acts on a list that stopped halfway. `max_tokens`, `pause_turn`, exhausted rounds and the wall clock each send a `notice` frame naming what happened, and the client also treats any `done` reason outside `end_turn` / `stop_sequence` as interrupted. `notice` is a separate frame from `error` for exactly this reason: the first version folded the message into `content`, which meant `content || message` dropped it whenever any text had already streamed, which is precisely when the warning matters. `MAX_TOKENS` is 16000, not 8000, because adaptive thinking spends from the same budget as ten ideas and ten citations.
- **`pause_turn` is a resume, not an end.** Anthropic pauses a turn that spends a while in web search and hands back the work so far, expecting the same content returned as-is. Breaking out of the loop on it threw the whole turn away and drew a blank answer. `MAX_PAUSES = 4`, and a resume does not count against `MAX_ROUNDS` because no tool result came back to this route.
- **Step lines are keyed by tool_use id, not matched on their text.** Each call is announced twice, once when its block opens before the arguments have streamed and once when they are known. Matching the refined label against the tail of the list worked until a turn called two tools, at which point the second placeholder was no longer last and both survived: four lines for two calls.
- **Citations are substituted by a recursive walk, not just in `p` and `li`.** The prompt asks for the citation on its own, but the model writes markdown, and the moment it bolds a line the citation sits inside a `<strong>`. Only handling direct children showed the visitor a raw `[[reel:...]]`.
- **The composer becomes a stop button while streaming**, and the hook aborts on unmount. Without it, leaving mid-answer left the agent loop running to completion against a connection nobody was reading, and it is billed either way.
- **`shortfallNote` measures which filter cut, it does not list which were set.** The first version named the per-creator cap on every shortfall, because the cap is always set, which sent the model to widen the one thing that was not the problem.
- Tests 84-87 in `tests/e2e/viral-reels.spec.ts` mock the stream on purpose: a length cut-off, a two-tool turn, a bolded citation and a stop all live entirely in the client and none can be provoked from a real answer on demand.
