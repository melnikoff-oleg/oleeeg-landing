...
- `/claude-code-instagram` — Claude Code as an Instagram video *editor* (added 2026-08-01). Companion to **Reel Studio** (`melnikoff-oleg/reel-studio`), a local app that drives Claude Code + the HyperFrames skills. Distinct from `/claude-reels`, which is about *writing* Reels scripts from competitor analysis; this one is about *cutting the video*. Nine accordion steps take a complete non-technical beginner from no VS Code to a rendered reel: VS Code → Node.js (22+, the renderer's floor) → a Claude plan (Pro minimum; the free plan has no Claude Code) → Claude Code (the CLI *and* the extension, with the "the extension does not put `claude` on your PATH" trap called out) → sign-in → ffmpeg + Python → clone/ZIP the repo → `npm install && npm run dev` on :4321 → first video. No `videoId` yet (the YouTube video is unpublished — add it to `page.tsx` + `layout.tsx` when it goes live, and only then add the route to `YT_FACADE_ROUTES` in `mobile-audit.spec.ts` and `SPECS` in `hero-cta.spec.ts`). Every install fact was verified against primary docs on 2026-08-01; the version numbers and winget package IDs will rot, so re-check before re-promoting the page.
